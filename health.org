...
(defvar health--prompt-askp nil) ; Hacky fix to org babel multi helm calls
(defun health--select-table ()
  (if health--prompt-askp
      (progn 
        (setq health--prompt-askp nil)
        (princ (format "table=%s" (health--prompt-table))))
    (progn
      (setq heatlh--prompt-askp t)
      (princ (format "table=%s" "\"Never occurs\"")))))
