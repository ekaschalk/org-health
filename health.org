...
(cl-defun health--read (HEADER OPTIONS &optional (FORMAT-FUNC #'-list))
  """HEADER prompts list of strings from FORMAT-FUNC applied to OPTIONS"""
  (if (string= "" OPTIONS)
...
(defun health--prompt-add (TABLE)
  ...
  (let ((headers (nth 0 TABLE))
        (options (nth 1 TABLE))
        (format-func (apply-partially #'s-split "/")))
    (--zip-with (health--read it other format-func) headers options)))

(defun health--add-to-table (TABLE)
  """Prompts and appends row to TABLE"""
  (append tab (list (health--prompt-add TABLE))))

(health--add-to-table exercises)
#+end_src

# #+NAME: add-to-tab
# #+begin_src emacs-lisp :var tab=tab-exercises :colnames no :hlines yes
# (health--add-to-table tab)
# #+end_src
